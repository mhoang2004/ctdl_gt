...
bool checkConsecutivePairs(vector <card> selectedCards)
{
    ...
    {
        if (selectedCards[i].getValue() == 15) return 0;
    }
    if (selectedCards.size() % 2 == 1) return 0;
    ...
    if (!checkStraight(straight_1)) return 0;
    if (!checkStraight(straight_2)) return 0;
    for (int i = 0; i < number_1; i++)
    {
        if (straight_1[i].getValue() != straight_2[i].getValue()) return 0;
    }
    return 1;
}
bool checkStraight(vector <card> selectedCards)
...
{  
    if (selectedCards.size() == 1) return 1;
    if (selectedCards.size() >= 6)
        if (checkConsecutivePairs(selectedCards) == 1) return 1;
    if (selectedCards.size() > 2)
        if (checkStraight(selectedCards)) return 1;
    if (selectedCards.size() >= 2)
        if (checkPairs(selectedCards) )return 1;
    return 0;
}
...
int main()
{
	vector <card> history ;
	card tmp;
	tmp.setValue(4); tmp.setSuits(5);
	history.push_back(tmp);
	tmp.setValue(5); tmp.setSuits(6);
	history.push_back(tmp);
	tmp.setValue(6); tmp.setSuits(6);
	history.push_back(tmp);
	for (int i=0; i<history.size(); i++)
		cout<<endl<<history[i];
	cout<<findSuitsMax(history);
	vector <card> users;
	tmp.setValue(5); tmp.setSuits(3);
	users.push_back(tmp);
	tmp.setValue(6); tmp.setSuits(4);
	users.push_back(tmp);
	tmp.setValue(7); tmp.setSuits(4);
	users.push_back(tmp);
	for (int i=0; i<users.size(); i++)
		cout<<endl<<users[i];
		cout<<findSuitsMax(users);

	cout<<endl<<"Ket Qua: "<<check_hit(history,users);
	return 0;
} 
